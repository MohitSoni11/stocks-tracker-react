...
function Account({ typeData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.get('type') || formData.get('name') === '') {
    ...
    if (result.success) {
      console.log('Added Account: ' + formData.get('name') + ' of type ' + formData.get('type'));
      ...
      <form onSubmit={handleSubmit}>
        <div className='bg-gray-800'>
          <div className='bg-gray-100 px-5 border-b border-gray-900/10 pb-12'>
            <h2 className='pt-5 text-base/7 font-semibold text-gray-900'>Add Account</h2>

            <div className='mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6'>
              <div className='sm:col-span-3'>
                <label htmlFor='type' className='block text-sm/6 font-medium text-gray-900'>Type</label>
                <div className='mt-2'>
                  <select name='type' id='type' className='block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6'>
                    {typeData.map((type) => (
