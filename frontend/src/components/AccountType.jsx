...
function AccountType({ fetchAccountTypes }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch('http://localhost:5000/add-account-type', {
      method: 'POST',
    ...
    if (result.success) {
      console.log('Added Account Type: ' + formData.get('name'));
      fetchAccountTypes();
      ...
      <form onSubmit={handleSubmit}>
        <div className='bg-gray-800'>
          <div className='bg-gray-100 px-5 border-b border-gray-900/10 pb-12'>
            <h2 className='pt-5 text-base/7 font-semibold text-gray-900'>Add Account Type</h2>
